...
export default function ReviewDraftsPage() {
  ...
  const fetchDrafts = async () => {
    try {
      ...
      const response = await fetch(`/api/history?type=review-draft&userId=${userId}`);
  ...
  const handleDelete = (id: string) => {
    if (confirm('确定要删除这项复盘任务吗？删除后无法恢复。')) {
      executeDelete(id);
    }
  };
  ...
  return (
    <div className="flex h-screen w-full bg-slate-50 font-sans antialiased text-slate-900 border-t-4 border-red-600 flex-col overflow-hidden">
          ...
          {isLoading ? (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              {filteredSessions.map((session) => {
                const checklist = JSON.parse(session.reviewChecklistData || '{}');
                const completedCount = Object.keys(checklist).length;

                return (
                  <Card key={session.id} className="group hover:border-red-300 transition-all duration-300 shadow-sm hover:shadow-xl bg-white rounded-3xl overflow-hidden border-2 border-slate-100">
                    <div className="p-6">
